...
function Introduction() {
  ...
  return (
    <div className="w-full flex rounded-xl px-2 py-1 my-5 max-sm:px-0">
      ...
      <div>
        <h1 className="text-left font-bold text-4xl flex items-center max-sm:text-2xl">
          Jhuvel Colina
          <BadgeCheck color="#1d9bf0" className="mx-3 size-5" />
        </h1>
        <p className="text-left flex items-center text-base max-sm:text-sm">
          <MapPin className="mx-2 size-5 max-sm:size-4" /> General Trias Cavite
        </p>
        <p className="text-left text-xl flex items-center mt-1 mb-2 max-sm:text-base">
          <BriefcaseBusiness className="mx-2 size-6 max-sm:size-5" /> Junior Web Developer
        </p>
        <p className="text-left flex items-center px-2 py-1 rounded-xl bg-[#1d9bf0] text-white font-bold hover:bg-gray-300 hover:text-black text-base max-sm:text-sm max-sm:px-1">
