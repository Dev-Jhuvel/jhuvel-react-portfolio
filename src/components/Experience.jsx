    ...
    return <div className="w-[30%] border border-gray-300 rounded-xl p-2 mb-5 max-sm:w-full animate-fade-left animate-delay-500 animate-duration-500 max-sm:animate-delay-1000">
        ...
        <ul className="list-none w-full">
           {experiences.map((value, index) => (
            <li className="font-bold text-xs flex justify-between text-left items-center my-2 bg-gray-100 hover:bg-gray-200 rounded-md px-3 py-1 cursor-pointer max-sm:text-sm" key={index} onMouseOver={() => setIcon(value.icon)} onMouseOut={() => setIcon(BriefcaseBusiness)} onClick={() => setIcon(value.icon)}>
                <span>{value.title}
